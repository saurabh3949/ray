...
from ray.rllib.contrib.bandits.models.logistic_regression import DiscreteLogisticModelThompsonSampling, \
    DiscreteLogisticModelUCB, DiscreteLogisticModel, ParametricLogisticModelThompsonSampling, ParametricLogisticModelUCB
...
def make_model_and_action_dist(policy, obs_space, action_space, config):
    ...
    model_cls = DiscreteLogisticModel
    ...
    if exploration_config:
        if exploration_config["type"] == ImplicitExploration.ThompsonSampling.name:
            if isinstance(original_space, spaces.Dict):
                ...
                model_cls = ParametricLogisticModelThompsonSampling
            else:
                model_cls = DiscreteLogisticModelThompsonSampling
        elif exploration_config["type"] == ImplicitExploration.UCB.name:
            if isinstance(original_space, spaces.Dict):
                assert "item" in original_space.spaces, "Cannot find 'item' key in observation space"
                model_cls = ParametricLogisticModelUCB
            else:
                model_cls = DiscreteLogisticModelUCB

    model = model_cls(obs_space, action_space, logit_dim, config["model"], name="LogisticModel")
...
GLMUCBTSPolicy = build_torch_policy(
    name="GLMUCBTSPolicy",
...
GLMTSTrainer = build_trainer(
    name="GLMTS",
    default_config=TS_CONFIG,
    default_policy=GLMUCBTSPolicy,
    collect_metrics_fn=get_stats)

GLMUCBTrainer = build_trainer(
    name="GLMUCB",
